...
class EffectState : IStatement
{
    ...
    public bool CheckSemantic(out List<string> errors)
    {
        ...
        try
        {
             if (this.name.Return == ExpressionType.String)
            {
    ...
    public void Take(List<(Token, IExpression)> _parameters, IExpression targets)
    {
        ...
        foreach (var parameter in _parameters)
        {
            try
            {
                ...
                else if (!(parameter.Item2.Return is ExpressionType.Object || parameter.Item2.Return == this._parameters[parameter.Item1.Value]))
                {
                    errors += $"Expected {this._parameters[parameter.Item1.Value]}, found {parameter.Item2.Return}\n";
                } 
            }
